...
async function checkStore(store) {
  try {
    ...
    const drinks = items.filter((item) => item.kind === "drink");
    const food = items.filter((item) => item.kind === "food");
    const flavourInStock = drinks.some((item) => item.inStock);
    const anyBanana = items.some((item) => item.inStock);
...
async function parseUkMenu(host, storeNumber) {
  ...
  while ((match = PRODUCT_RE.exec(html))) {
    ...
    if (!/banana/i.test(name)) continue;
    found.set(match[3], {
      id: match[3],
      name,
      inStock: match[1] === "false",
      kind: isFood(name) ? "food" : "drink",
    });
  }

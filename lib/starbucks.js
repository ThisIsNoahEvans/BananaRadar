...
export async function searchBanana({ lat, lng, q, onProgress } = {}) {
  const emit = (event) => {
  ...
  emit({ type: "origin", origin, market });

  const stores = await findStores(origin.lat, origin.lng, market);
  const nearest = stores.slice(0, STORE_LIMIT);
  emit({ type: "stores", origin, market, stores: nearest });

  const checked = await mapLimit(nearest, 4, async (store, index) => {
    const result = await checkStore(store);
    emit({ type: "store", store: result, index });
    return result;
  });
...
async function resolveOrigin({ lat, lng, q }) {
  ...
  if (ALMOST_UK_POSTCODE.test(query)) {
    if (!UK_POSTCODE.test(query)) {
      throw new Error(
        "That UK postcode looks incomplete. Use the full postcode, or try a town name."
      );
    }
...
async function geocodeUkPostcode(postcode) {
  ...
  try {
    ...
    if (err?.status === 404 || err?.status === 400) {
      throw new Error(
        "Couldn't find that UK postcode. Check it, or try a town or place name."
      );
    }
    throw lookupError(err, "Couldn't look up that postcode.");
  }
...
async function geocodePlace(query) {
  ...
  try {
    ...
    throw lookupError(err, "Couldn't look up that place.");
  }
  if (!Array.isArray(results) || !results[0]) {
    throw new Error(
      ALMOST_UK_POSTCODE.test(query)
        ? "Couldn't find that UK postcode. Check it, or try a town or place name."
        : "Couldn't find that place. Try a postcode, town, or area name."
    );
  }
  const hit = results[0];
  return {
...
async function findStores(lat, lng, market) {
  const attempts =
    market === "uk" ? [findUkStores, findUsStores] : [findUsStores, findUkStores];
  let lastError;
  let sawEmpty = false;
  for (const attempt of attempts) {
    try {
      const stores = await attempt(lat, lng);
  ...
  throw lookupError(
    lastError,
    "Couldn't find nearby Starbucks right now. Try again in a moment."
  );
}
...
function lookupError(err, fallback) {
  const message = String(err?.message || "");
  if (/took too long|couldn't reach the lookup/i.test(message)) {
    return err instanceof Error ? err : new Error(fallback);
  }
  if (err?.status === 429) {
    return new Error("Too many searches. Try again in a moment.");
  }
...
async function fetchText(url, init = {}, timeoutMs = 10000) {
  ...
  try {
    ...
    if (err?.name === "TimeoutError") {
      throw new Error("That lookup took too long. Try again in a moment.");
    }
    if (err?.name === "AbortError") throw err;
    throw new Error("Couldn't reach the lookup service. Try again in a moment.");
  }
